...
import { PACE_TYPES, getSuggestedSaveDC } from "@/utils/threatModifiers";
...
export default function Step8_Actions() {
  ...
  // State for custom pace value input
  const [customPaceValue, setCustomPaceValue] = useState<number>(2);
  
  // Auto-propagate to-hit bonus based on area/targets
  useEffect(() => {
    if (actionForm.area) {
      const isAreaAttack = actionForm.area.toLowerCase().includes('area') || 
                          actionForm.area.toLowerCase().includes('all') ||
                          actionForm.area.toLowerCase().includes('multiple') ||
                          actionForm.area.toLowerCase().includes('each') ||
                          (actionForm.area.toLowerCase() !== 'one creature' && 
                           actionForm.area.toLowerCase() !== 'one target');
      
      const bonusToHit = isAreaAttack ? threat.attack?.area : threat.attack?.single;
      if (bonusToHit !== undefined && bonusToHit !== actionForm.toHit) {
        setActionForm(prev => ({ ...prev, toHit: bonusToHit }));
      }
    }
  }, [actionForm.area, threat.attack]);
                ...
                {actionForm.type === "Attack" && (
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Suggested Save DC for {threat.rank} rank: <span className="font-medium">{getSuggestedSaveDC(threat.rank)}</span>
                    </p>
                  </div>
                )}
                  ...
                  <div className="space-y-2 mt-2">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => setCustomPaceValue(parseInt(e.target.value) || 2)}
                        data-testid="input-pace-value"
                      />
                      <Label className="text-xs text-muted-foreground self-center">areas</Label>
                    </div>
                  </div>
                </div>
              </div>
